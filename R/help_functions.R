...
include <- function(I, data) {
  ...
  if (!is.null(I)) {
      ...
      if (class(I.orig) %in% c("numeric", "integer")) {
        ...
        if (!all(I.orig %in% 1:length(f1.no.y.string))) {
          ...
          stop(paste("'formula' was determined to have ", length(f1.no.y.string),
            ...
            paste(I.orig[!index.in], collapse = ",", sep = " "), ".",
            sep = ""
          ))
        }
      }
    ...
    if ("formula" %in% class(I)) {
      if (!("Formula" %in% class(I))) {
        if (length(I) > 2L) {
          stop("'I' must be a one-sided formula, e.g. '~ x1 + x2'.")
        }
      } else {
        if (length(I)[1] > 0L) {
          stop("'I' must be a one-sided formula: '~ x1 + x2' is ok, 'y ~ x1 + x2' is not.")
        }
        if (length(I)[2] > 1L) {
          stop("'I' must be a one-sided formula, '~ x1 + x2' is ok, '~ x1 + x2 | z1 + z2' is not.")
        }
      }
    }
