...
rlassoEffects.default <- function(x, y, index = c(1:ncol(x)), method = "partialling out",
  ...
  for (i in 1:k) {
    ...
    I3m <- I3[-index[i]]
    lasso.regs[[i]] <- try(col <- rlassoEffect(Xt, y, d,
      method = method,
      I3 = I3m, post = post, ...
    ), silent = TRUE)
...
rlassoEffects.formula <- function(formula, data, I, method = "partialling out",
  ...
  est <- rlassoEffects(x, y,
    index = I.c, method = method,
    I3 = I3, post = post, ...
  )
  est$call <- cl
  return(est)
...
rlassoEffect <- function(x, y, d, method = "double selection", I3 = NULL,
  ...
  if (method == "partialling out") {
    ...
    names(I) <- union(names(I1), names(I2))
    results <- list(
      alpha = unname(alpha), se = drop(se), t = unname(tval),
...
plot.rlassoEffects <- function(x, joint = FALSE, level = 0.95, main = "", xlab = "coef", ylab = "",
  ...
  plotobject <- plotobject + ggplot2::geom_errorbar(
    ymin = coefmatrix$lower,
    ymax = coefmatrix$upper, colour = col
  )

  # further graphic parameter
  plotobject <- plotobject + ggplot2::ggtitle(main) + ggplot2::ylim(
    low,
    up
...
#' data <- data.frame(cbind(y, X))
#' colnames(data)[1] <- "y"
#' lasso.effect <- rlassoEffects(X, y,
#'   index = c(1, 2, 3, 50),
