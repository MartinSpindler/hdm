...
manual_double_selection <- function(data, x_cols, post){
  fmla.y <- as.formula(paste('y ~ ',paste(x_cols, collapse='+')))
  fmla.d <- as.formula(paste('d ~ ',paste(x_cols, collapse='+')))
  rY <- rlasso(fmla.y, data=data, post=post)$coefficients
  rD <- rlasso(fmla.d, data=data, post=post)$coefficients
  
  sel_x = xnames[abs(betaY[xnames])>0 | abs(betaD[xnames])>0]
  fmla.double.sel <- as.formula(paste('y ~ ', paste(c("d", sel_x), collapse='+')))
  partial.fit <- lm(fmla.double.sel, sim_data)
  return(partial.fit)
}
...
patrick::with_parameters_test_that("Unit tests for rlassoEffect vs manual:", .cases = test_cases, {
  ...
  if (method == 'partialling out') {
    ...
    partial.fit <- manual_double_selection(sim_data, xnames, post=post)
  }
  ...
  if (method == 'partialling out') {
    ...
    expect_equal(partial.fit$coefficients['d'], Eff$alpha, tolerance = 1e-8)
  }
}
)
